...
public class UIFloat : MonoBehaviour {
	...
	public Vector3 posDir = new Vector3(0, 1, 0), scaleDir = new Vector3(0, 0, 0), rotDir = new Vector3(0, 0, 0), posOffset, rotOffset, scaleOffset;
	...
	void Update () {
		targetPos = basePos + posDir * Mathf.Sin(Time.time * posFreq) * posAmp + posOffset;
		targetScale = baseScale + scaleDir * Mathf.Cos(Time.time * scaleFreq) * scaleAmp + scaleOffset;
		...
		if(rotDir != Vector3.zero && rotate) {
			targetRot = baseRot + rotDir * Mathf.Sin(Time.time * rotFreq) * rotAmp + rotOffset;
